...
/* ========= Шаблоны PowerShell (без изменений) ========= */
const templates = [
...
function exportData() {
  ...
  const a    = Object.assign(document.createElement('a'),
                             { href:url, download:'bot-memory.json' });
...
function trainFromText() {
  ...
  if (!txt) return alert('Сначала вставь текст!');
...
function analysis(){
  ...
  corpus.forEach(s => s.toLowerCase().split(/[^\\p{L}0-9]+/u)
                   .forEach(w => { if (!w || stopW.has(w)) return;
                                   total++; freq[w] = (freq[w]||0)+1; }));
...
const kb  = q => { let best=null,score=0;
...
async function ask(){
  ...
  if (q.includes(' - ')) {
    ...
    if (req && ans) {
      knowledgeBase.push({ request:req.toLowerCase(), answer:ans });
      saveAll(); updateProgress();
