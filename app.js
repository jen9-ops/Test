...
const templateMatch=q=>{for(const t of templates){const m=q.match(t.pattern);
  if(m)return typeof t.script==='function'?t.script(m.groups||{}):t.script;}return null;};
...
function updateProgress(){
  document.getElementById('progress').textContent=
    Math.min(100,Math.round(corpus.length/TARGET*100))+' %';
}
updateProgress();
...
async function neuralScript(prompt){
  ...
  return out[0].generated_text.split('```powershell')[1]
         ?.replace('```','')?.trim();
}
...
function append(sender,text,cls){
  ...
  d.className=`msg ${cls}`;d.textContent=`${sender}: ${text}`;
  ...
  r.onload=ev=>{
    try{
      ...
      if(Array.isArray(d.knowledgeBase)&&Array.isArray(d.corpus)){
        knowledgeBase=d.knowledgeBase; corpus=d.corpus;
...
function trainFromText(){
  ...
  saveAll(); updateProgress(); textInput.value='';
  append('ИИ',`Обучено: ${s.length}`,'bot');
}
async function trainFromURL(){
  const url=urlInput.value.trim(); if(!url){alert('URL?'); return;}
  try{
...
const kb=q=>{let best=null,s=0;knowledgeBase.forEach(e=>{
  const sc=sim(q,e.request); if(sc>s){s=sc; best=e;}});
...
async function ask(){
  ...
  if(q.includes(' - ')){const[req,ans]=q.split(' - ').map(s=>s.trim());
    if(req&&ans){knowledgeBase.push({request:req.toLowerCase(),answer:ans});
                 saveAll(); updateProgress(); append('ИИ','Запомнил!','bot');}
    return;}
